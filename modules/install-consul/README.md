...
[run-consul script](../run-consul) to create a Consul [an Openstack Glance Image](https://docs.openstack.org/glance/latest/) that can be deployed in [OVH Public Cloud](https://www.ovh.com/fr/public-cloud/instances/) across a Server Group using the [consul-cluster module](../consul-cluster).
...
terraform--consul/modules/install-consul/install-consul --version 0.8.0
```

The `install-consul` script will install Consul, its dependencies, and the [run-consul script](https://github.com/ovh/terraform-ovh-publiccloud-consul/tree/master/modules/run-consul).
...
We recommend running the `install-consul` script as part of a [Packer](https://www.packer.io/) template to create a Consul [Glance Image](https://docs.openstack.org/glance/latest/) (see the 
[consul-glance-image example](../../examples/consul-glance-image) for a fully-working sample code). You can then deploy the image across a Server Group using the [consul-cluster module](../../modules/consul-cluster) (see the [main ](../../MAIN.md) for fully-working sample code).
...
See [Package Managers](https://github.com/ovh/terraform-ovh-publiccloud-consul/tree/master/_docs/package-managers.md) for a full discussion of the requirements, trade-offs, and why we
picked `git`.
